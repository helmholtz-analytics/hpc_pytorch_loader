...
class DenseMemmapReader(Reader):
    """
    ...
    def __init__(self, dataset_path=None, transform=None, target_transform=None):
        """
        Initialize the MemmapReader class.

        This class is designed to read images and labels from memory-mapped files.
        It supports both distributed and non-distributed data processing.

        Parameters
        ----------
        dataset_path : str, optional
            Path to the dataset directory. Default is None.
    ...
    def _read_single_array(self, image, label, array_length):
        ...
        label_array = np.memmap(label_path, dtype=np.uint8, mode='r', shape=(array_length, *shape))
    ...
    def __getitem__(self, idx):
        ...
        label = Image.fromarray(np.array(self.labels_list[list_ind][arr_ind]))
