...
class IpcConverter(ConverterFlexibleSize):
    ...
    def _custom_collate_fn(batch):
        ...
        labels_schema = pa.schema([pa.field('label', pa.int64())])
        ...
        for (image, label) in batch:
            ...
            labels_array = pa.array([label], type=pa.int64())
            # Create a record batch for the label
            labels_batch = pa.record_batch([labels_array], schema=labels_schema)
            # Add the record batch to the labels list
